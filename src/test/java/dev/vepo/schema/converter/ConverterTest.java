...
public class ConverterTest {
    ...
    class Avro {
        ...
        @Test
        void fullObjectTest() throws IOException {
            Converter converter = new Converter();
            Parser parser = new Parser();
            Schema original = parser.parse(getClass().getResourceAsStream("/avro/sample-full.json"));
            Schema converted = converter.toAvro(new JsonSchema(getClass().getResourceAsStream("/json/sample-full.json"))).toAvro();
            assertEquals(original, converted);
        }
    }
}
